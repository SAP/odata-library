...
class Service {
      ...
      value: new Promise((resolve, reject) => {
        this.agent
          ...
          .then((responses) => {
            ...
            extensions = this.buildExtensions(metadata, settings);
            resolve();
          })
          .catch(reject);
      }),
    ...
    return metadata.model.getSchema().actions.reduce((acc, metaAction) => {
      ...
      if (metaAction.isBound) {
        Object.values(entitySets)
          ...
          .forEach((s) => s.addAction(action, agent));
      } else {
  ...
  buildExtensions(metadata, settings) {
    const schema = metadata.model.getSchema();
    const arrayConcat = (acc, val) => acc.concat(val);
    const adaptsettings = (relativePath) => {
      let newSettings;
      const relativePathRx = /^((?:\.\.\/)+)(.*)\/\$metadata$/;
      if (relativePathRx.test(relativePath)) {
        const [all, prefix, suffix] = relativePathRx.exec(relativePath);
        const mainServicePathRx = new RegExp(`^(.+)${prefix.replaceAll(/\.\.\//g, "\/[^\/]+")}\/?$`);
        if (mainServicePathRx.test(settings.url)) {
          const [all2, root] = mainServicePathRx.exec(settings.url);
          newSettings = _.cloneDeep(settings);
          delete newSettings.logger; // TODO some better solution
          newSettings.url = `${root}/${suffix}`;
          newSettings.delegate = this.agent;
        }
      }
    ...
    // It should be always dedicated service for VH and Field, according "Core Data Services for ABAP", 2nd edition 2022, but it is not specification.
    const valueListReferences = schema.entityTypes
    .map(et => et.properties
        .map(p => p.sap.valueListReferences || []) // TODO handle v2
        .reduce(arrayConcat)
    ).reduce(arrayConcat, []);

    const valueListReferencePaths = _.uniq(valueListReferences.map(r => r.path));
    const relatedServicesSettings = valueListReferencePaths.map(p => ({
      path: p,
      settings: adaptsettings(p)
    })).filter(r => r.settings)
    .reduce((acc, serviceInfo) => {
      acc[serviceInfo.path] = serviceInfo.settings;
      return acc;
    }, {});

    valueListReferences.forEach(r => Object.defineProperty(r, "service", {
      get: () => new Service(relatedServicesSettings[r.path]),
    }));

    // TODO what to return?
    return {
      sap: {
        valueListReferences: valueListReferences
      }
    };
  }
