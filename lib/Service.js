...
class Service {
      ...
      value: new Promise((resolve, reject) => {
        this.agent
          ...
          .then((responses) => {
            ...
            actions = this.buildActions(this.agent, metadata, entitySets);
            resolve();
          })
          .catch(reject);
      }),
  ...
  buildActions(agent, metadata, entitySets) {
    return metadata.model.getSchema().actions.reduce((acc, metaAction) => {
      ...
      if (metaAction.isBound) {
        Object.values(entitySets)
          .filter((s) => s.entityTypeModel === metaAction.boundType)
          .forEach((s) => {
            s.actions.push(action);
          });
      } else {
        acc[metaAction.name] = action;
        if (!this[metaAction.name]) {
          this[metaAction.name] = action.createDirectCaller();
        } else {
          agent.logger.warn(
            `Action ${metaAction.name} is not accessible as shorthand.`
          );
        }
      }
