...
class Agent {
   ...
   * @memberof Agent
   */
  metadata(prefix = this.prefix) {
    let metadataUrls = _.concat(
      //Core metadata data url
      [
        `${prefix}/$metadata?${this.metadataSearch(
    ...
    if (this.settings.delegate) {
      return Promise.all(
        _.map(metadataUrls, (requestUrl) =>
          this.settings.delegate.createMetadataRequest(requestUrl)
        )
      ).then((responses) => {
   ...
   * @memberof Agent
   */
  sendRequest(httpMethod, inputUrl, headers, payload, prefix = this.prefix) {
    if (this.settings.delegate) {
      return this.settings.delegate.sendRequest(httpMethod, inputUrl, headers, payload, this.prefix);
    }
    ...
    let requestUrl = `${url.normalize(inputUrl, prefix)}`;
  ...
  fetchToken() {
    if (this.settings.delegate) {
      return this.settings.delegate.fetchToken();
    }
