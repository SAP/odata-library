...
class EntitySet extends QueryableResource {
   ...
   * @memberof EntitySet
   */
  addAction(action, agent) {
    this.actions.push(action);
    if (action.meta.boundType.elementType === this.entityTypeModel) {
      if (!this[action.meta.name]) {
        this[action.meta.name] = action.createDirectCaller(this);
      } else {
        agent.logger.warn(
          `Bound Action ${action.meta.name} is not accessible as shorthand on ${this.entitySetModel.name} entity set.`
        );
      }
    }
  }
