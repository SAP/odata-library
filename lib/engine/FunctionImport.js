...
class FunctionImport extends Resource {
  ...
  post() {
    ...
    if (defaultBatch) {
      ...
      callRequestPromise = this.agent.fetchToken().then((csrfToken) => {
        if (csrfToken) {
  ...
  normalizeResponse(rawResponse) {
    ...
    } else if (_.isArray(_.get(rawResponse, this.agent._listResultPath))) {
      response = _.get(rawResponse, this.agent._listResultPath);
    } else if (_.has(rawResponse, this.agent._instanceResultPath)) {
      response = _.get(rawResponse, this.agent._instanceResultPath);
    } else {
      throw new Error("Invalid oData response from backend");
    }
    return response;
  }
