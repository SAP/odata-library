...
function createNamespaceRegEx(count) {
  let pattern = '^(';
...
function getNamespace(path,count) {
  let regEx = createNamespaceRegEx(count);
...
class EdmxModel {
   ...
   * @memberof EdmxModel
   */
  resolveModelPath(path,count=1) {
    let namespace = getNamespace(path,count);
    if (!namespace) {
      throw new Error(`Can't find schema for '${path}' model path.`);
    }
    let schema = this.getSchema(namespace);
    if (!schema) {
      return this.resolveModelPath(path,count+1)
    }
