...
function applyEntityTypeExtensions(entityType) {
  ...
  entityType.properties.forEach((prop) => {
    const vhRefs = (_.get(prop.annotations.find(a => a.term === "SAP__common.ValueListReferences"), "collection") || []).map(path => ({
      path: path
    }));
    Object.defineProperty(prop, "sap", {
      get: () => ({
        sortable: true, // no longer used, SortRestrictions to be used
        valueListReferences: vhRefs
      }),
    });
  });
}
