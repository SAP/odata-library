...
class Action extends AnnotationTarget {
  ...
  initSchemaDependentProperties(schema) {
    ...
    if (this.isBound) {
      let boundType = this.parameters.find(
        (p) => p.name === this.boundTypeParameterName
      ).type;
